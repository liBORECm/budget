...
class FooService extends CRUDService<FooBase, Foo> {
    ...
    // Override getAll() with getAllEnriched to enrich the base record with relations if necessary
    public async getAll(
        ...
        return super.getAllEnriched((record) => {
...
| Method | Description |
|---|---|
| `getAll(modifier?, sort?, offset?, limit?, enrich?)` | All non-deleted rows; `modifier` is a Knex query callback |
